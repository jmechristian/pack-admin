...
import { listCPSForms } from '@/graphql/queries';
...
const getData = async () => {
  const forms = await API.graphql({ query: listCPSForms });
  return { forms };
};
          ...
          <div className='inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8'>
            <table className='min-w-full divide-y divide-gray-300'>
              ...
              <tbody className='divide-y divide-gray-200'>
                {forms.data.listCPSForms.items.map((person) => (
                  <tr key={person.email}>
